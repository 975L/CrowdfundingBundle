...
class SkillsTest extends TestCase
{
    ...
    private const string SLUG_PATTERN = '#^(crowdfunding|shop|site)-[a-z-]+$#';

    private const string NAME_PATTERN = '#^[a-z][a-z0-9]*(_[a-z0-9]+)+(\([^)]*\))?$#';

    private const string COMMAND_PATTERN = '/c975l:[a-z0-9:-]+/';

    private const string COMPONENT_PATTERN = '/<twig:c975LCrowdfunding:([A-Za-z:]+)/';
    ...
    // No "ui.block" is tagged here today - this bundle hosts the other bundles' kinds on its own entity rather than declaring any - but the day it does, the skill's names are checked against them like everybody else's
    ...
    // This bundle declares no config slug of its own: every one it reads belongs to a dependency, which is exactly what the check below is for
    ...
    public function testEveryCommandQuotedExists(): void
    {
        ...
        foreach ($this->loadSkills() as $directory => $content) {
            ...
            foreach (array_unique($matches[0]) as $command) {
                ...
                // This bundle names its own command under the c975l:crowdfunding: prefix, so the pattern above cannot be scoped to a prefix of its own - a command it does not declare is checked against the ecosystem, exactly as a config slug is
