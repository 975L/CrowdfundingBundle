...
namespace c975L\CrowdfundingBundle\Tests\Translation;
...
{
    // The bundle's own narrations, resolved in its "crowdfunding" domain suffixed "_narration" - spoken and never drawn, hence the two locales the rest of the bundle does not stop at
    ...
    public function testEveryDeclaredNarrationIsShippedInEveryLocale(): void
    {
        ...
        foreach (self::LOCALES as $locale) {
            ...
            foreach ($declared as $file => $keys) {
                foreach ($keys as $key) {
                    $this->assertContains($key, $catalogue, sprintf('"%s" declares "%s", which "crowdfunding_narration.%s.xlf" does not ship.', $file, $key, $locale));
                }
            }
        }
    }
    ...
    public function testNoNarrationIsShippedWithoutBeingDeclared(): void
    {
        ...
        foreach ($this->catalogue('en') as $key) {
            $this->assertContains($key, $declared, sprintf('"crowdfunding_narration.en.xlf" ships "%s", which nothing in "src/" declares any more.', $key));
        }
    }
    ...
    {
        $path = $this->root() . '/translations/crowdfunding_narration.' . $locale . '.xlf';
