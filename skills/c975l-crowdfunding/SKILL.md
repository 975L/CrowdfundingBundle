...
description: "Use this skill when working on crowdfunding campaigns or the lottery of a c975L site — a campaign and its counterparts, contributing through the basket, the news an author publishes, the campaign's media and videos, and the lottery tied to a campaign with its prizes, its tickets and its draw. Covers what the basket copies of a counterpart, when a contributor row is written, why a ticket is drawn only once, and how a campaign answers and is translated in every language of the site. Triggers on: Crowdfunding, CrowdfundingCounterpart, CrowdfundingContributor, CrowdfundingContributorCounterpart, CrowdfundingNews, CrowdfundingMedia, KIND_HERO, KIND_SLIDE, getHeroes, getSlides, mediasOfKind, CrowdfundingVideo, Lottery, LotteryPrize, LotteryTicket, LotteryVideo, CrowdfundingBasketItemProvider, CrowdfundingService, CrowdfundingCounterpartService, LotteryService, crowdfunding_index, crowdfunding_display, crowdfunding_preview, lottery_display, lottery_draw_prize, drawWinner, generateTicketsForContributor, generateTicketNumber, validateAddition, validateCheckout, onBasketValidated, onBasketPaid, limitedQuantity, orderedQuantity, lotteryTickets, amountGoal, amountAchieved, beginDate, endDate, hidden, isDeleted, setIsDeleted, deleteEntity, deletePermanently, restore, trashAction, writeGoneRedirect, CrowdfundingEmailTemplateProvider, CrowdfundingEmailFactory, CrowdfundingEmailSender, crowdfunding_contribution, lottery_tickets, lottery_ticket_winner, CrowdfundingSitemapProvider, SocialContentSourceInterface, CrowdfundingSocialContentSource, CrowdfundingNewsSocialContentSource, SocialContent, getRepeatAfterDays, getNextContent, crowdfunding_news, CrowdfundingBackupPathProvider, CrowdfundingBlockOwnerResolver, crowdfunding_media, CrowdfundingGuidedProjectProvider, ProcedureProvider, procedures.json, _suggestions.html.twig, crowdfunding-counterpart-titles, crowdfunding-counterpart-prices, crowdfunding-prize-titles, label.counterpart_name_scout, label.prize_name_diamond, filmer-tirage-loterie, publier-actualite-campagne, crowdfunding_narration, crowdfunding_slider, crowdfunding_counterparts, crowdfunding_lottery, crowdfunding_campaigns, crowdfunding_block_campaign, crowdfunding_block_campaigns, crowdfunding_block_sheet_kinds, crowdfunding_funding_state, CrowdfundingBlockExtension, CrowdfundingFundingExtension, CrowdfundingEditExtension, crowdfunding_edit_url, SliderBlockType, CounterpartsBlockType, LotteryBlockType, CampaignsBlockType, CrowdfundingBlockCacheTagProvider, CrowdfundingBlockCacheInvalidator, CrowdfundingCacheInvalidationListener, CrowdfundingBlockEditUrlProvider, CrowdfundingShowcaseProvider, CrowdfundingDemoFixtureProvider, DemoFixtureProviderInterface, demo fixtures, CrowdfundingFilesHealthCheckProvider, files-crowdfunding, FundingBar, Hero, UseFor, highlightedSlug, lowStockThreshold, showTicketsCount, showUse, trans_default_domain, orphanRemoval, updateEntity, site-role-editor, site-role-admin, CrowdfundingTranslator, CrowdfundingTranslatedLocales, CrowdfundingLinkLocalizer, CrowdfundingTranslationBuilder, CrowdfundingTranslationPurgeListener, translatedLocales, setTranslated, getUntranslated, translation_locale, crowdfunding_index_localized, crowdfunding_display_localized, lottery_display_localized, localized_path, ContentLocaleScreen, LocalizedRouteNegotiator, bareRoute, crowdfunding-translate, hreflang, language screen."
---
...
`src/Entity/Crowdfunding.php`, `src/Entity/CrowdfundingCounterpart.php`, `src/Entity/CrowdfundingContributor.php`, `src/Entity/Lottery.php`, `src/Entity/LotteryPrize.php`, `src/Entity/LotteryTicket.php`, `src/Service/CrowdfundingBasketItemProvider.php`, `src/Service/LotteryService.php`, `src/Service/CrowdfundingService.php`, `src/Controller/CrowdfundingController.php`, `src/Controller/LotteryController.php`, `src/Controller/Management/CrowdfundingCrudController.php`, `src/Email/CrowdfundingEmailTemplateProvider.php`, `src/Email/CrowdfundingEmailFactory.php`, `src/Email/CrowdfundingEmailSender.php`, `src/Management/CrowdfundingSitemapProvider.php`, `src/Service/CrowdfundingSocialContentSource.php`, `src/Service/CrowdfundingNewsSocialContentSource.php`, `src/Management/CrowdfundingBlockOwnerResolver.php`, `src/Management/CrowdfundingGuidedProjectProvider.php`, `src/Management/ProcedureProvider.php`, `src/Management/CrowdfundingBlockEditUrlProvider.php`, `src/Management/CrowdfundingFilesHealthCheckProvider.php`, `src/Twig/Extension/CrowdfundingBlockExtension.php`, `src/Twig/Extension/CrowdfundingFundingExtension.php`, `src/Twig/Extension/CrowdfundingEditExtension.php`, `src/Entity/CrowdfundingMedia.php`, `src/Service/CrowdfundingBlockCacheTagProvider.php`, `src/Service/CrowdfundingBlockCacheInvalidator.php`, `src/Service/CrowdfundingShowcaseProvider.php`, `src/Service/CrowdfundingDemoFixtureProvider.php`, `src/Listener/CrowdfundingCacheInvalidationListener.php`, `src/Form/Block/`, `templates/blocks/`, `config/services.yaml`, `config/procedures.json`, `templates/crowdfunding/`, `templates/lottery/`, `templates/emails/slots/`, `src/Service/CrowdfundingTranslator.php`, `src/Service/CrowdfundingTranslatedLocales.php`, `src/Service/CrowdfundingLinkLocalizer.php`, `src/Form/Util/CrowdfundingTranslationBuilder.php`, `src/Listener/CrowdfundingTranslationPurgeListener.php`, `templates/management/crowdfunding_crud_edit.html.twig`
...
- `Management\CrowdfundingSitemapProvider` declares the index and one url per campaign; a finished campaign is rated as the archive it has become. On a multilingual site each page is declared once per language with its `alternates`, a campaign's group naming only the languages `CrowdfundingTranslator::translatedLocales()` returns.
- `Service\CrowdfundingSocialContentSource` (UiBundle's `SocialContentSourceInterface`, source type `crowdfunding`) hands SocialBundle's publication the campaigns open today — `beginDate` reached, `endDate` not passed, a campaign without dates never offered — each carrying its cover or its hero, and offered again after 30 days. `Service\CrowdfundingNewsSocialContentSource` (source type `crowdfunding_news`) does the same for their news of the last 30 days, most recent first, borrowing the campaign's page and image and linking to `#news-<id>`, never offered twice. Both are picked up automatically and stay silent on a site without SocialBundle.
- `Service\CrowdfundingDemoFixtureProvider` (UiBundle's `DemoFixtureProviderInterface`) seeds a demo site with one open campaign — pictures keyed `crowdfunding/<slug>` by the site, the generic placeholder pool otherwise — three tiers, a news, a lottery with one prize drawn at the campaign's end, and eight contributors; `amountAchieved` and each tier's `orderedQuantity` are summed from the contributors, never typed, and the visitor-facing texts come from `label.sample_*` keys, translated on the second pass.
...
**None of them stores what to show, only how.** The medias, the tiers and the draws are read from the campaign's own rows at render time through `crowdfunding_block_campaign()` (`Twig\Extension\CrowdfundingBlockExtension`), which resolves the campaign off the `crowdfunding_display` or `crowdfunding_preview` route being rendered, bare or localised (`LocalizedRouteNegotiator::bareRoute()`) — so a block never goes stale against the funding, no form ever asks which campaign to show, and one of these kinds placed anywhere else renders nothing at all.
...
**The bar** — the whole CRUD sits behind `site-role-editor`, as SiteBundle's pages do: index, form, entity permission, QR code, *view on site*, *preview* and *translate*. Only `restore()` and `deletePermanently()` are stricter, on `site-role-admin`, and so is `LotteryController::drawPrize()`.
...
On a site declaring several languages, `crowdfunding_index_localized`, `crowdfunding_display_localized` and `lottery_display_localized` double the three bare routes. They answer in **every** language the site declares, translated or not — `Service\CrowdfundingTranslatedLocales` holds that rule, read by the controllers through ConfigBundle's `LocalizedRouteNegotiator` — and a template links to them through `localized_path()`, `Service\CrowdfundingLinkLocalizer` doing the same for a stored link or a rich text. Code comparing the current route reads it through `LocalizedRouteNegotiator::bareRoute()`, or it stops recognising the localised twin.

`Service\CrowdfundingTranslator` translates what an editor types, beside the row rather than in a second campaign: `CrowdfundingTranslator::CAMPAIGN_FIELDS`, `CrowdfundingTranslator::COUNTERPART_FIELDS`, `CrowdfundingTranslator::NEWS_FIELDS` and `CrowdfundingTranslator::PRIZE_FIELDS`, filed under their own owner in UiBundle's translation table. A slug, an amount, a name and a ticket number are never translated.

- **Translate at render, never on `postLoad`.** `CrowdfundingTranslator::apply()` lays a language over each row for the render being built; the getters then read the translation and `getUntranslated()` the text the row was written with. The back office runs in the editor's language and must keep showing the source text, so whatever renders a row on the front calls `apply()` on it — the campaign, its counterparts, its news and the prizes of its lotteries.
- **Which url answers is not which language a row exists in.** `CrowdfundingTranslatedLocales::forCrowdfunding()` answers every language the site declares; `CrowdfundingTranslator::translatedLocales()` answers the languages the row's own name is written in, and is what an "hreflang" group may name.
- **The language screen** is the campaign's edit screen opened on another language, from the tabs of `templates/management/crowdfunding_crud_edit.html.twig` or the list's *Translate* action. It offers the campaign's texts unmapped, then its collections through their own form types with the `translation_locale` option (`Form\Util\CrowdfundingTranslationBuilder`), never adding nor removing a row; what it writes is stored on the flush saving the campaign.
- `Listener\CrowdfundingTranslationPurgeListener` deletes a row's translations with the row — nothing else would, a translation naming its owner without a foreign key.
- The `crowdfunding-translate` guided project walks that screen.
